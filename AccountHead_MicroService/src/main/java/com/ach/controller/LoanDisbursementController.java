...
public class LoanDisbursementController
{
	...
	public ResponseEntity<Ledger> generateLedger(@RequestBody Ledger ledger, @PathVariable int CustomerRegId)
	{
		...
		cureg.getLedger().setTenure(tenure);
		...
		cureg.getLedger().setAmountPaidTillDate(cureg.getSanctionLetter().getMonthlyEMIAmount());
		ledger.setAmountPaidTillDate(cureg.getSanctionLetter().getMonthlyEMIAmount());
		cureg.getLedger().setRemainingAmount(Math.round(ledger.getPayableAmountWithInterest()-ledger.getAmountPaidTillDate()));
		ledger.setRemainingAmount(Math.round(ledger.getPayableAmountWithInterest()-ledger.getAmountPaidTillDate()));
		...
		cureg.getLedger().setCurrentMonthEmiStatus(cureg.getLoanDisbursement().getPaymentStatus());
		cureg.getLedger().setLoanEndDate(ledger.getLoanEndDate());
		cureg.getLedger().setLoanStatus(cureg.getLoanStatus());
		disServiceI.saveRegForm(cureg);
		EmailDetails ed=new EmailDetails();
		ed.setToEmail(cureg.getEmailId());
